...
public class AccountActivity extends AppCompatActivity implements
    ...
    public boolean mBackToSpash = false;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                Log.w(TAG, "Credential read failed.");
            }
    ...
    private void handleGoogleSignIn(GoogleSignInResult gsr) {
        ...
        if (isSignedIn) {
            ...
            if (gsa.getPhotoUrl() != null)
                new LoadProfileImage(imageView).execute(gsa.getPhotoUrl().toString() + "?sz=400");

            ((TextView) findViewById(R.id.profile_email)).setText(gsa.getEmail());
            ((TextView) findViewById(R.id.profile_name)).setText(gsa.getDisplayName());
            ...
            ImageView imageView = (ImageView) findViewById(R.id.profile_pic);
            imageView.setImageResource(R.drawable.guernica);

            ((TextView) findViewById(R.id.profile_email)).setText(R.string.contact_mail);
            ((TextView) findViewById(R.id.profile_name)).setText(R.string.app_name);

            Intent intent = new Intent();
            intent.putExtra(BACK_TO_LOGIN, true);
...
//        findViewById(R.id.button_google_revoke).setEnabled(isSignedIn);
    ...
    private void resolveResult(Status status, int requestCode) {
        if (!mIsResolving) {
            try {
                status.startResolutionForResult(AccountActivity.this, requestCode);
                mIsResolving = true;
